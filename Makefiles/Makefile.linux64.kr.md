# `Makefile.linux64` 빌드 분석 (한국어)

> MemtestG80을 **64비트 Linux**에서 빌드하는 GNU Make 파일입니다. 두 소스(`memtestG80_core.cu`,
> `memtestG80_cli.cu`)와 헤더(`memtestG80_core.h`)를 `nvcc`로 컴파일·링크해 실행 파일 `memtestG80`을 만듭니다.
> 이 문서는 빌드 구조를 블록 다이어그램과 함께 설명합니다.

- 원본: [`Makefile.linux64`](Makefile.linux64) · 저장소 루트에서 `make -f Makefiles/Makefile.linux64` 로 실행
- 플랫폼: **64비트 Linux** · 산출물: `memtestG80` · 목적 파일 확장자: `.o`
...
| 변수 | 값 | 설명 |
|---|---|---|
| `ZLIB_DIR` | `zlib/linux64` | zlib 라이브러리 경로 (-L로 전달) |
| `CUDA_DIR` | `/usr/local/cuda/lib64` | CUDA 런타임 라이브러리 경로 (-L로 전달) |
| `NVCC` | `nvcc` | CUDA 컴파일러 드라이버 (실제 빌드에 사용) |
| `CXX` | `g++` | C++ 컴파일러 변수 (선언되지만 아래 규칙에서 사용되지 않음) |
| `CFLAGS` | `-DLINUX -O -Wall -m64` | C 플래그 변수 (선언되지만 사용되지 않음) |
| `NVCCFLAGS` | `-DLINUX -L$(ZLIB_DIR) -L$(CUDA_DIR) -O2 -Xptxas -v -Xcompiler -Wall -m64` | nvcc에 전달되는 실제 컴파일·링크 플래그 |
...
`-DLINUX -L$(ZLIB_DIR) -L$(CUDA_DIR) -O2 -Xptxas -v -Xcompiler -Wall -m64`

| 플래그 | 의미 |
|---|---|
| `-DLINUX` | 플랫폼 매크로 — 소스가 OS별 코드를 선택 (core.h의 #if defined 분기) |
| `-O2` | 최적화 레벨 2 (nvcc 호스트/디바이스 코드) |
| `-Xptxas -v` | PTX 어셈블러에 상세 출력 요청 → 커널별 레지스터·공유 메모리 사용량 표시 |
| `-L$(ZLIB_DIR) -L$(CUDA_DIR)` | 링커 라이브러리 검색 경로 추가 |
| `-Xcompiler -Wall` | 호스트 컴파일러(g++)에 -Wall(모든 경고) 전달 |
| `-m64` | 타깃 아키텍처 비트폭 (64비트) |
...
```bash
# 저장소 루트에서
make -f Makefiles/Makefile.linux64          # memtestG80 빌드
make -f Makefiles/Makefile.linux64 clean    # 목적 파일·실행 파일 정리
```

- `nvcc`가 PATH에 있어야 하고, CUDA 프로그램을 실행할 수 있는 환경이어야 합니다.
- 빌드 실패의 흔한 원인은 `ZLIB_DIR`/`CUDA_DIR` 경로가 실제 설치 위치와 다른 경우입니다 (파일 상단에서 수정).

> **플랫폼 참고**: 32비트 대응 파일은 Makefile.linux32 (경로가 lib32/linux32, 아키텍처가 -m32)입니다. 4GB 초과 메모리 GPU를 다루려면 64비트 빌드가 필요합니다.
...
> **↳** 이 파일(`Makefile.linux64`)은 위 표의 **64비트 Linux** 열에 해당합니다.
